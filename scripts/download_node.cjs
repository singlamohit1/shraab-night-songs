        ...
        try {
            ...
            if (search.length > 0) {
                ...
                await new Promise((resolve, reject) => {
                    const writeStream = fs.createWriteStream(`public/songs/song_${i+1}.webm`);
                    const audioStream = ytdl(search[0].url, { filter: 'audioonly' });
                    audioStream.pipe(writeStream);
                    writeStream.on('finish', () => {
                        console.log(`Saved song_${i+1}.webm`);
                        resolve();
                    });
